...
public:
    ...
    Move get_move(const Board& board)
    {
        while (_move_list.size() > 0)
        {
            ...
            _move_list.pop_back();
            if (board.is_move_legal(next_move))
            {
                return next_move;
            }
            else
    ...
    {
        const uint64_t double_push_rank = board.player == Player::white ? bitboard::rank_3 : bitboard::rank_6;
        ...
        // Double pawn push.
        uint64_t double_push_mask = move_pawns(single_push_mask & double_push_rank, empty_mask, forward_delta, generator);
